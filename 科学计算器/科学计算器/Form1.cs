...
namespace 科学计算器
{
    public partial class Form1 : Form
    {
        ...
        {
            introduction = "    这个科学计算器可以实现加，减，乘，除，求幂，开方，阶乘，取余操作以及";
            introduction += "三角函数和对数函数的应用.为了简化，目前还没加入进制转换功能，某些时候";
            introduction += "表达式的输入也有可能不太符合习惯，因此请尽量输入正确的表达式哈，";
            introduction += "有时间我会对上述不足进行改进的~\n    主要思想是用栈和队列来进行计算，";
            introduction += "代码中被注释部分是无意中发现了可以使用eval()函数直接计算表达式，不是偷懒..";
            introduction += "\n    这是第一次做这样的一个项目,水平还很有限，";
            introduction += "有什么错误请尽管指出来，谢谢O(∩_∩)O~~";

//***********************用eval()函数实现时使用该语句******************************************
            //cal.Language = "javascript";   
//*********************************************************************************************

            MessageBox.Show(introduction,"介绍");
        }
